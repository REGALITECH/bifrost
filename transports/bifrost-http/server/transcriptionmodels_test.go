...
func TestTranscriptionCatalogRegistrationPreservesState(t *testing.T) {
	...
	require.NoError(t, s.UpsertModelPricingAttributes(ctx, []handlers.ModelPricingAttributesEntry{{Provider: "transcription", Model: "asr", Enabled: schemas.Ptr(false)}}))
	...
	require.Empty(t, catalog.GetModelsForProvider(schemas.Transcription))
