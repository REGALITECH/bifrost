...
```json
{
  ...
  "config": {
    ...
    "customer_mapping": { "BIFROST_VIRTUAL_KEY_UUID": "METRONOME_CUSTOMER_ID" }
  }
}
```
...
An ingest error, missing customer mapping, shutdown, or configured plugin that
...
Build the gateway (see above), configure the explicit Sandbox mapping, and
restart with the Sandbox key in the process environment.
